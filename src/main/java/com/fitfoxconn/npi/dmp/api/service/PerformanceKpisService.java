...
public class PerformanceKpisService {
   ...
   * 抓取多個設備，某天的效率相關KPI
   */
  public GetSingleDatedPerformanceKpisOutput getSingleDatedPerformanceKpis(List<Integer> equipmentIds,
      LocalDate queryDate,
      ShiftEnum shiftEnum) {
    ...
    //取得良率目標 from pz_Line
    Map<String, String> distinctLineCode = new HashMap<>();
    equipmentInfoMap.values().forEach(v -> {
      if (!distinctLineCode.containsKey(v.getLineOrganizationCode())) {
        distinctLineCode.put(v.getLineOrganizationCode(), v.getLineOrganizationCode());
      }
    });
    List<PzLineEntity> pzLineEntityList = this.pzLineDao.findByLineOrganizationCodeIn(
        distinctLineCode.values().stream().toList());
